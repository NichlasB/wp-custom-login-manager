 ...
 */
function wpclm_uninstall_single_site() {
    ...
    $options = array(
        ...
        'wpclm_message_too_many_attempts'
    );
...
// Handle multisite vs single site uninstall
if (is_multisite()) {
    global $wpdb;
    $blog_ids = $wpdb->get_col("SELECT blog_id FROM {$wpdb->blogs}");
    foreach ($blog_ids as $blog_id) {
        switch_to_blog($blog_id);
        wpclm_uninstall_single_site();
...
// Delete debug log file if it exists (once, not per-site)
// Use stored randomized log name if available, fallback to legacy name
$debug_log_name = get_option('wpclm_debug_log_name', '');
if (!empty($debug_log_name)) {
    $debug_log = WP_CONTENT_DIR . '/' . $debug_log_name;
    if (file_exists($debug_log)) {
        unlink($debug_log);
    }
    delete_option('wpclm_debug_log_name');
}
// Also clean up legacy hardcoded log file if it exists
$legacy_debug_log = WP_CONTENT_DIR . '/wpclm-debug.log';
if (file_exists($legacy_debug_log)) {
    unlink($legacy_debug_log);
}
